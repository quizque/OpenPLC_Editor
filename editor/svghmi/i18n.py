...
def open_pofile(pofile):
    ...
    if sys.platform.startswith("win"):
        ...
        try:
            poedit_path = subprocess.check_output(
                "command -v poedit", shell=True
            ).strip()
...
def MatchTranslations(translations, messages, errcallback):
    ...
    for msgid, label, svgid in messages:
        ...
        for langcode, translation in translations:
            ...
            if msg is None:
                ...
                errcallback(
                    _('{}: Missing translation for "{}" (label:{}, id:{})\n').format(
                        langcode, msgid, label, svgid
                    )
                )
    ...
    for langcode, translation in translations:
        ...
        for msgid, msg in translation.items():
            broken = True
            errcallback(
                _('{}: Unused translation "{}":"{}"\n').format(langcode, msgid, msg)
            )
        if broken or langcode in broken_lang:
            errcallback(
                _(
                    'Translation for {} is outdated, please edit {}.po, click "Catalog -> Update from POT File..." and select messages.pot.\n'
