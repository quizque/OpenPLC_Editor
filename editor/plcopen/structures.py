...
def GetSubTypes(type):
    ...
    return [
        typename
        for typename, _parenttype in TypeHierarchy.items()
                    ...
                    for outype in output_types:
                        ...
                        for InTypes, OutTypes in ANY_TO_ANY_FILTERS.get(
                            ...
                            outs = reduce(
                                lambda a, b: a or b,
                                map(
                                    lambda testtype: IsOfType(
                                        Function_decl["outputs"][0][1], testtype
                                    ),
                                    OutTypes,
                                ),
                            )
                            inps = reduce(
                                ...
                                map(
                                    lambda testtype: IsOfType(
                                        Function_decl["inputs"][0][1], testtype
                                    ),
                                    InTypes,
                                ),
                            )
...
TYPE_KEYWORDS.extend(
    [keyword for keyword in TypeHierarchy.keys() if keyword not in TYPE_KEYWORDS]
)
