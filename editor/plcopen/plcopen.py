...
PLCOpen_v1_xsd = etree.XMLSchema(etree.fromstring(PLCOpen_v1_xml.encode("utf-8")))
    ...
    def getpous(self, exclude=None, filter=None):
        ...
        return self.xpath(
            ...
            % (
                ...
                ("[%s]" % " or ".join(map(lambda x: "@pouType='%s'" % x, filter)))
                if len(filter) > 0
                else "",
            ),
    ...
    def gettypeAsText(self):
        ...
        # Variable type is an array
            ...
            return "ARRAY [%s] OF %s" % (
                ",".join(
                    map(
                        lambda x: "%s..%s" % (x.getlower(), x.getupper()),
                        vartype_content.getdimension(),
                    )
                ),
                basetype_name,
            )
