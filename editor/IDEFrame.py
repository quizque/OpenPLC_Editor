...
[
    ID_PLCOPENEDITOR,
    ID_PLCOPENEDITORLEFTNOTEBOOK,
    ID_PLCOPENEDITORBOTTOMNOTEBOOK,
    ID_PLCOPENEDITORRIGHTNOTEBOOK,
    ID_PLCOPENEDITORPROJECTTREE,
    ID_PLCOPENEDITORMAINSPLITTER,
    ID_PLCOPENEDITORSECONDSPLITTER,
    ID_PLCOPENEDITORTHIRDSPLITTER,
    ID_PLCOPENEDITORLIBRARYPANEL,
    ID_PLCOPENEDITORLIBRARYSEARCHCTRL,
    ID_PLCOPENEDITORLIBRARYTREE,
    ID_PLCOPENEDITORLIBRARYCOMMENT,
    ID_PLCOPENEDITORTABSOPENED,
    ID_PLCOPENEDITORTABSOPENED,
    ID_PLCOPENEDITOREDITORMENUTOOLBAR,
...
[
    ID_PLCOPENEDITOREDITMENUENABLEUNDOREDO,
    ID_PLCOPENEDITOREDITMENUADDDATATYPE,
    ID_PLCOPENEDITOREDITMENUADDFUNCTION,
    ID_PLCOPENEDITOREDITMENUADDFUNCTIONBLOCK,
    ID_PLCOPENEDITOREDITMENUADDPROGRAM,
    ID_PLCOPENEDITOREDITMENUADDCONFIGURATION,
    ID_PLCOPENEDITOREDITMENUFINDNEXT,
    ID_PLCOPENEDITOREDITMENUFINDPREVIOUS,
...
[
    ID_PLCOPENEDITOREDITORTOOLBARSELECTION,
    ID_PLCOPENEDITOREDITORTOOLBARCOMMENT,
    ID_PLCOPENEDITOREDITORTOOLBARVARIABLE,
    ID_PLCOPENEDITOREDITORTOOLBARBLOCK,
    ID_PLCOPENEDITOREDITORTOOLBARCONNECTION,
    ID_PLCOPENEDITOREDITORTOOLBARWIRE,
    ID_PLCOPENEDITOREDITORTOOLBARPOWERRAIL,
    ID_PLCOPENEDITOREDITORTOOLBARRUNG,
    ID_PLCOPENEDITOREDITORTOOLBARCOIL,
    ID_PLCOPENEDITOREDITORTOOLBARCONTACT,
    ID_PLCOPENEDITOREDITORTOOLBARBRANCH,
    ID_PLCOPENEDITOREDITORTOOLBARINITIALSTEP,
    ID_PLCOPENEDITOREDITORTOOLBARSTEP,
    ID_PLCOPENEDITOREDITORTOOLBARTRANSITION,
    ID_PLCOPENEDITOREDITORTOOLBARACTIONBLOCK,
    ID_PLCOPENEDITOREDITORTOOLBARDIVERGENCE,
    ID_PLCOPENEDITOREDITORTOOLBARJUMP,
    ID_PLCOPENEDITOREDITORTOOLBARMOTION,
...
def AppendMenu(parent, help, kind, text, id=wx.ID_ANY):
    return parent.Append(helpString=help, kind=kind, item=text, id=id)
...
def SimplifyTabLayout(tabs, rect):
    for tab in tabs:
        if tab["pos"][0] == rect.x:
            ...
            for other in others:
                if (
                    other["pos"][1] == tab["pos"][1]
                    and other["size"][1] == tab["size"][1]
                    and other["pos"][0] == tab["pos"][0] + tab["size"][0] + TAB_BORDER
                ):
                    tab["size"] = (
                        tab["size"][0] + other["size"][0] + TAB_BORDER,
                        tab["size"][1],
                    )
            ...
            for other in others:
                if (
                    other["pos"][0] == tab["pos"][0]
                    and other["size"][0] == tab["size"][0]
                    and other["pos"][1] == tab["pos"][1] + tab["size"][1] + TAB_BORDER
                ):
                    tab["size"] = (
                        tab["size"][0],
                        tab["size"][1] + other["size"][1] + TAB_BORDER,
                    )
                    tab["pages"].extend(other["pages"])
                    tabs.remove(other)
    ...
    for idx, tab in enumerate(tabs):
        ...
        if tab["size"][0] == rect.width:
            if tab["pos"][1] == rect.y:
                ...
                split_rect = wx.Rect(
                    rect.x,
                    rect.y + tab["size"][1] + TAB_BORDER,
                    rect.width,
                    rect.height - tab["size"][1] - TAB_BORDER,
                )
            ...
            if tab["pos"][0] == rect.x:
                ...
                split_rect = wx.Rect(
                    rect.x + tab["size"][0] + TAB_BORDER,
                    rect.y,
                    rect.width - tab["size"][0] - TAB_BORDER,
                    rect.height,
                )
...
class IDEFrame(wx.Frame):
    ...
    def InitEditorToolbarItems(self):
        ...
        self.EditorToolBarItems = {
            "FBD": [
                (
                    True,
                    FREEDRAWING_MODE | DRIVENDRAWING_MODE,
                    ID_PLCOPENEDITOREDITORTOOLBARMOTION,
                    "OnMotionTool",
                    "move",
                    _("Move the view"),
    ...
    def _init_coll_EditMenu_Items(self, parent):
        ...
        AppendMenu(
            ...
            kind=wx.ITEM_NORMAL,
            text=_("Find Previous") + "\tCTRL+SHIFT+K",
        )
        parent.AppendSeparator()
        AppendMenu(
            parent,
            ...
            kind=wx.ITEM_NORMAL,
            text=_("Search in Project") + "\tCTRL+SHIFT+F",
        )
        parent.AppendSeparator()
        ...
        AppendMenu(
            ...
            id=wx.ID_SELECTALL,
            kind=wx.ITEM_NORMAL,
            text=_("Select All") + "\tCTRL+A",
        )
        AppendMenu(
            parent, help="", id=wx.ID_DELETE, kind=wx.ITEM_NORMAL, text=_("&Delete")
        )
        ...
        self.Bind(
            wx.EVT_MENU,
            self.OnFindPreviousMenu,
            id=ID_PLCOPENEDITOREDITMENUFINDPREVIOUS,
        )
        self.Bind(
            wx.EVT_MENU,
            self.OnSearchInProjectMenu,
            id=ID_PLCOPENEDITOREDITMENUSEARCHINPROJECT,
        )
        self.Bind(
            wx.EVT_MENU, self.OnAddDataTypeMenu, id=ID_PLCOPENEDITOREDITMENUADDDATATYPE
        )
        self.Bind(
            wx.EVT_MENU,
            self.GenerateAddPouFunction("function"),
            id=ID_PLCOPENEDITOREDITMENUADDFUNCTION,
        )
        self.Bind(
            wx.EVT_MENU,
            self.GenerateAddPouFunction("functionBlock"),
            id=ID_PLCOPENEDITOREDITMENUADDFUNCTIONBLOCK,
        )
        self.Bind(
            wx.EVT_MENU,
            self.GenerateAddPouFunction("program"),
            id=ID_PLCOPENEDITOREDITMENUADDPROGRAM,
        )
        self.Bind(
            wx.EVT_MENU, self.AddResourceMenu, id=ID_PLCOPENEDITOREDITMENUADDRESOURCE
        )
        self.Bind(
            wx.EVT_MENU,
            self.OnAddConfigurationMenu,
            id=ID_PLCOPENEDITOREDITMENUADDCONFIGURATION,
        )
    ...
    def _init_coll_DisplayMenu_Items(self, parent):
        ...
        AppendMenu(
            ...
            kind=wx.ITEM_NORMAL,
            text=_("Full screen") + "\tShift-F12",
        )
        self.Bind(
            wx.EVT_MENU,
            self.SwitchFullScrMode,
            id=ID_PLCOPENEDITORDISPLAYMENUFULLSCREEN,
        )
        ...
        AppendMenu(
            ...
            kind=wx.ITEM_NORMAL,
            text=_("Reset Perspective"),
        )
        self.Bind(
            wx.EVT_MENU,
            self.OnResetPerspective,
            id=ID_PLCOPENEDITORDISPLAYMENURESETPERSPECTIVE,
        )
        ...
        self.alphasortMenuItem = AppendMenu(
            parent, help="", id=sort_alpha_id, kind=wx.ITEM_CHECK, text=_("Sort Alpha")
        )
    ...
    def _init_ctrls(self, prnt):
        ...
        self.LeftNoteBook.Bind(
            wx.aui.EVT_AUINOTEBOOK_ALLOW_DND, self.OnAllowNotebookDnD
        )
        ...
        self.TabsOpened.Bind(
            wx.aui.EVT_AUINOTEBOOK_PAGE_CHANGING, self.OnPouSelectedChanging
        )
        self.TabsOpened.Bind(
            wx.aui.EVT_AUINOTEBOOK_PAGE_CHANGED, self.OnPouSelectedChanged
        )
        self.TabsOpened.Bind(wx.aui.EVT_AUINOTEBOOK_PAGE_CLOSE, self.OnPageClose)
        self.TabsOpened.Bind(wx.aui.EVT_AUINOTEBOOK_END_DRAG, self.OnPageDragged)
        self.AUIManager.AddPane(
            self.TabsOpened, wx.aui.AuiPaneInfo().CentrePane().Name("TabsPane")
        )
        ...
        self.ProjectPanel = wx.SplitterWindow(
            ...
            name="ProjectPanel",
            parent=self.LeftNoteBook,
            pos=wx.Point(0, 0),
            size=wx.Size(0, 0),
            style=wx.SP_3D,
        )

        self.ProjectTree = CustomTree(
        ...
        self.ProjectTree.SetBackgroundBitmap(
            GetBitmap("custom_tree_background"), wx.ALIGN_RIGHT | wx.ALIGN_BOTTOM
        )
        add_menu = wx.Menu()
        self._init_coll_AddMenu_Items(add_menu)
        ...
        self.Bind(
            ...
            self.OnProjectTreeItemChanging,
            id=ID_PLCOPENEDITORPROJECTTREE,
        )
        self.Bind(
            wx.EVT_TREE_BEGIN_DRAG,
            self.OnProjectTreeBeginDrag,
            id=ID_PLCOPENEDITORPROJECTTREE,
        )
        self.Bind(
            wx.EVT_TREE_BEGIN_LABEL_EDIT,
            self.OnProjectTreeItemBeginEdit,
            id=ID_PLCOPENEDITORPROJECTTREE,
        )
        self.Bind(
            wx.EVT_TREE_END_LABEL_EDIT,
            self.OnProjectTreeItemEndEdit,
            id=ID_PLCOPENEDITORPROJECTTREE,
        )
        self.Bind(
            wx.EVT_TREE_ITEM_ACTIVATED,
            self.OnProjectTreeItemActivated,
            id=ID_PLCOPENEDITORPROJECTTREE,
        )
        ...
        self.Bind(
            wx.EVT_MENU, self.OnSelectionTool, id=ID_PLCOPENEDITOREDITORTOOLBARSELECTION
        )
        ...
        if self.EnableDebug:
            self.DebugVariablePanel = DebugVariablePanel(
                self.RightNoteBook, self.Controler, self
            )
            self.MainTabs["DebugVariablePanel"] = (
                self.DebugVariablePanel,
                _("Debugger"),
            )
    ...
    def __init__(self, parent, enable_debug=False):
        ...
        notebooks = {}
        for notebook, entry_name in [
            (self.LeftNoteBook, "leftnotebook"),
        ...
        if page_infos[0] == "main":
            ...
            if instance_infos is not None:
                return notebook.GetPageIndex(
                    self.OpenDebugViewer(
                        instance_infos["class"], instance_path, instance_infos["type"]
                    )
                )
        return None
    ...
    def OnPreviewMenu(self, event):
        ...
        if selected != -1:
            ...
            margins = (
                self.PageSetupData.GetMarginTopLeft(),
    ...
    def OnPrintMenu(self, event):
        ...
        if selected != -1:
            ...
            if (
                not printer.Print(self, printout, True)
                and printer.GetLastError() != wx.PRINTER_CANCELLED
            ):
        ...
        for child in self.TabsOpened.GetChildren():
            if isinstance(child, wx.aui.AuiTabCtrl):
                ...
                if child not in self.AuiTabCtrl:
                    child.Bind(
                        wx.EVT_LEFT_DCLICK, self.GetTabsOpenedDClickFunction(child)
                    )
    ...
    def RecursiveProjectTreeItemSelection(self, root, items):
        ...
        while item is not None and item.IsOk() and not found:
            ...
            if (item_infos["name"].split(":")[-1].strip(), item_infos["type"]) == items[
                0
            ]:
                if len(items) == 1:
    ...
    def OnProjectTreeBeginDrag(self, event):
        selected_item = (
            self.SelectedItem if self.SelectedItem is not None else event.GetItem()
        )
        if (
            selected_item.IsOk()
            and self.ProjectTree.GetPyData(selected_item)["type"] == ITEM_POU
        ):
        ...
        if new_name != "":
            if not TestIdentifier(new_name):
                ...
                if item_infos["type"] == ITEM_PROJECT:
                    ...
                    if new_name.upper() in [
                        name.upper()
                        for name in self.Controler.GetProjectDataTypeNames()
                        if name != old_name
                    ]:
                    ...
                    if not abort:
                        ...
                        self.RefreshEditorNames(
                            ComputeDataTypeName(old_name), ComputeDataTypeName(new_name)
                        )
                        self.RefreshPageTitles()
                elif item_infos["type"] == ITEM_POU:
                    if new_name.upper() in [
                        name.upper()
                        for name in self.Controler.GetProjectPouNames()
                        if name != old_name
                    ]:
                        message = _('"%s" pou already exists!') % new_name
                        abort = True
                    elif new_name.upper() in [
                        name.upper()
                        for name in self.Controler.GetProjectPouVariableNames()
                    ]:
                        messageDialog = wx.MessageDialog(
                            self,
                            _(
                                'A POU has an element named "%s". This could cause a conflict. Do you wish to continue?'
                            )
                            % new_name,
                            _("Error"),
                    ...
                    if not abort:
                        ...
                        self.RefreshEditorNames(
                            ComputePouName(old_name), ComputePouName(new_name)
                        )
                    ...
                    if new_name.upper() in [
                        name.upper() for name in self.Controler.GetProjectPouNames()
                    ]:
                        message = _('A POU named "%s" already exists!') % new_name
                    elif new_name.upper() in [
                        name.upper()
                        for name in self.Controler.GetProjectPouVariableNames(pou_name)
                        if name != old_name
                    ]:
                        message = (
                            _(
                        ...
                        self.RefreshEditorNames(
                            ComputePouTransitionName(words[1], old_name),
                    ...
                    if new_name.upper() in [
                        ...
                        self.RefreshEditorNames(
                            ComputePouActionName(words[1], old_name),
                    ...
                    if new_name.upper() in [
                        name.upper()
                        for name in self.Controler.GetProjectConfigNames()
                        if name != old_name
                    ]:
                        ...
                        messageDialog = wx.MessageDialog(
                            self,
                            _(
                                'There is a POU named "%s". This could cause a conflict. Do you wish to continue?'
                            )
                            % new_name,
                            _("Error"),
                            wx.YES_NO | wx.ICON_QUESTION,
                    ...
                    if not abort:
                        ...
                        self.RefreshEditorNames(
                            ComputeConfigurationName(old_name),
                            ComputeConfigurationName(new_name),
                        )
                        self.RefreshPageTitles()
                elif item_infos["type"] == ITEM_RESOURCE:
                    if new_name.upper() in [
                        name.upper() for name in self.Controler.GetProjectConfigNames()
                    ]:
                        message = _('"%s" config already exists!') % new_name
                        abort = True
                    elif new_name.upper() in [
                    ...
                    if not abort:
                        ...
                        self.RefreshEditorNames(
                            ComputeConfigurationResourceName(words[1], old_name),
        ...
        if not event.Dragging():
            ...
            if item is not None and item.IsOk() and flags & wx.TREE_HITTEST_ONITEMLABEL:
                ...
                if item != self.LastToolTipItem and self.LastToolTipItem is not None:
                    ...
                    self.LastToolTipItem = None
                if self.LastToolTipItem != item and item_infos["type"] in [
                    ITEM_POU,
                    ITEM_TRANSITION,
                    ITEM_ACTION,
                ]:
                    ...
                    wx.CallAfter(
                        self.ProjectTree.SetToolTipString,
                        "%s : %s : %s" % (block_type, bodytype, item_infos["name"]),
                    )
    ...
    def OnProjectTreeItemChanging(self, event):
        if (
            self.ProjectTree.GetPyData(event.GetItem())["type"] not in ITEMS_UNEDITABLE
            and self.SelectedItem is None
        ):
    ...
    def OnProjectTreeRightUp(self, event):
        ...
        if item_infos["type"] in ITEMS_UNEDITABLE + [ITEM_PROJECT]:
            ...
            if name == "Data Types":
                menu = wx.Menu(title="")
                new_item = AppendMenu(
                    menu, help="", kind=wx.ITEM_NORMAL, text=_("Add DataType")
                )
                ...
                if name != "Project":
                    new_item = AppendMenu(
                        menu, help="", kind=wx.ITEM_NORMAL, text=_("Add POU")
                    )
                    self.Bind(
                        wx.EVT_MENU,
                        self.GenerateAddPouFunction(
                            {
                                "Functions": "function",
                ...
                new_item = AppendMenu(
                    menu, help="", kind=wx.ITEM_NORMAL, text=_("Paste POU")
                )
                ...
                menu = wx.Menu(title="")
                new_item = AppendMenu(
                    menu, help="", kind=wx.ITEM_NORMAL, text=_("Add Configuration")
                )
                ...
                menu = wx.Menu(title="")
                new_item = AppendMenu(
                    menu, help="", kind=wx.ITEM_NORMAL, text=_("Add Transition")
                )
                ...
                self.Bind(
                    wx.EVT_MENU,
                    self.GenerateAddTransitionFunction(
                        self.ProjectTree.GetItemText(parent)
                    ),
                    new_item,
                )
                ...
                menu = wx.Menu(title="")
                new_item = AppendMenu(
                    menu, help="", kind=wx.ITEM_NORMAL, text=_("Add Action")
                )
                parent = self.ProjectTree.GetItemParent(item)
                parent_type = self.ProjectTree.GetPyData(parent)["type"]
                while parent_type != ITEM_POU:
                ...
                self.Bind(
                    wx.EVT_MENU,
                    self.GenerateAddActionFunction(
                        self.ProjectTree.GetItemText(parent)
                    ),
                    new_item,
                )
                ...
                menu = wx.Menu(title="")
                new_item = AppendMenu(
                    menu, help="", kind=wx.ITEM_NORMAL, text=_("Add Resource")
                )
                parent = self.ProjectTree.GetItemParent(item)
                parent_type = self.ProjectTree.GetPyData(parent)["type"]
                ...
                if parent_name is not None:
                    self.Bind(
                        wx.EVT_MENU,
                        self.GenerateAddResourceFunction(parent_name),
                        new_item,
                    )
            ...
            if item_infos["type"] == ITEM_POU:
                ...
                if self.Controler.GetPouBodyType(name) == "SFC":
                    ...
                    self.Bind(
                        wx.EVT_MENU, self.GenerateAddTransitionFunction(name), new_item
                    )
                    new_item = AppendMenu(
                        menu, help="", kind=wx.ITEM_NORMAL, text=_("Add Action")
                    )
                    self.Bind(
                        wx.EVT_MENU, self.GenerateAddActionFunction(name), new_item
                    )
                    menu.AppendSeparator()

                new_item = AppendMenu(
                    menu, help="", kind=wx.ITEM_NORMAL, text=_("Copy POU")
                )
                ...
                if pou_type in ["function", "functionBlock"]:
                    ...
                    if pou_type == "function":
                        ...
                        self.Bind(
                            wx.EVT_MENU,
                            self.GenerateChangePouTypeFunction(name, "functionBlock"),
                            new_item,
                        )
                    new_item = AppendMenu(
                        change_menu, help="", kind=wx.ITEM_NORMAL, text=_("Program")
                    )
                    self.Bind(
                        wx.EVT_MENU,
                        self.GenerateChangePouTypeFunction(name, "program"),
                        new_item,
                    )
                    menu.AppendMenu(wx.ID_ANY, _("Duplicate as..."), change_menu)
                new_item = AppendMenu(
                    menu, help="", kind=wx.ITEM_NORMAL, text=_("Rename")
                )
            ...
            if menu is not None:
                new_item = AppendMenu(
                    menu, help="", kind=wx.ITEM_NORMAL, text=_("Delete")
                )
    ...
    def CloseObsoleteDebugTabs(self):
        if self.EnableDebug:
            ...
            for idx in idxs:
                ...
                elif editor.IsDebugging() and hasattr(
                    editor, "SubscribeAllDataConsumers"
                ):
                    editor.SubscribeAllDataConsumers()
    ...
    def AddToMenuToolBar(self, items):
        ...
        for toolbar_item in items:
            if toolbar_item is None:
                ...
                MenuToolBar.AddSimpleTool(id, GetBitmap(bitmap), help)
    ...
    def RefreshEditorToolBar(self):
        ...
        if menu is not None and menu != self.CurrentMenu:
            ...
            EditorToolBar = self.Panes["EditorToolBar"]
            if EditorToolBar:
                for radio, modes, id, method, picture, help in self.EditorToolBarItems[
                    menu
                ]:
    ...
    def GenerateAddPouFunction(self, pou_type, type_readonly=False):
        def OnAddPouMenu(event):
            ...
            dialog.SetValues(
                {
                    "pouName": self.Controler.GenerateNewName(
                        None, None, "%s%%d" % pou_type
                    )
                }
            )
            pou_created = False
    ...
    def GenerateAddTransitionFunction(self, pou_name):
        def OnAddTransitionMenu(event):
            ...
            dialog.SetValues(
                {
                    "transitionName": self.Controler.GenerateNewName(
                        None, None, "transition%d"
                    )
                }
            )
            if dialog.ShowModal() == wx.ID_OK:
    ...
    def OnAddConfigurationMenu(self, event):
        ...
        if tagname is not None:
            self._Refresh(
                TITLE, FILEMENU, EDITMENU, PROJECTTREE, POUINSTANCEVARIABLESPANEL
            )
    ...
    def GenerateChangePouTypeFunction(self, name, new_type):
        def OnChangePouTypeMenu(event):
            ...
            if self.ProjectTree.GetPyData(selected)["type"] == ITEM_POU:
                ...
                self._Refresh(
                    TITLE, EDITORTOOLBAR, FILEMENU, EDITMENU, PROJECTTREE, LIBRARYTREE
                )
    ...
    def OnPastePou(self, event):
        ...
        if self.ProjectTree.GetPyData(selected)["type"] != ITEM_PROJECT:
            ...
            pou_type = self.UNEDITABLE_NAMES_DICT[
                pou_type
    ...
    def CheckDataTypeIsUsedBeforeDeletion(self, name):
        return self.CheckElementIsUsedBeforeDeletion(
            self.Controler.DataTypeIsUsed, _("Remove Datatype"), name
        )

    def CheckPouIsUsedBeforeDeletion(self, name):
        return self.CheckElementIsUsedBeforeDeletion(
            self.Controler.PouIsUsed, _("Remove Pou"), name
        )
    ...
    def ClearHighlights(self, highlight_type=None):
        if highlight_type is None:
            ...
            self.Highlights = dict(
                [
                    (name, highlight)
                    for name, highlight in self.Highlights.items()
...
class GraphicPrintout(wx.Printout):
    def __init__(self, viewer, page_size, margins, preview=False):
        ...
        self.PageGrid = (
            UPPER_DIV(maxx, self.PageSize[0]),
