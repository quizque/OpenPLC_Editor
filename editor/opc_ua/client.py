...
Open62541IncludePaths = [
    os.path.join(Open62541Path, *dirs)
    for dirs in [
        ("plugins", "include"),
        ("build", "src_generated"),
        ("include",),
...
class OPCUAClientEditor(ConfTreeNodeEditor):
    CONFNODEEDITOR_TABS = [(_("OPC-UA Client"), "CreateOPCUAClient_UI")]
    ...
    def CreateOPCUAClient_UI(self, parent):
        return OPCUAClientPanel(
            parent, self.Controler.GetModelData(), self.Log, self.UriGetter
        )
...
class OPCUAClient(object):
    ...
    def CTNGenerate_C(self, buildpath, locations):
        ...
        c_code = self.modeldata.GenerateC(
            c_path, locstr, self.GetParamsAttributes("OPCUAClient.Server_URI")["value"]
        )
                ...
                entries.append(
                    {
                        "name": dname,
                        "type": {
                            "input": LOCATION_VAR_INPUT,
                            "output": LOCATION_VAR_OUTPUT,
                        }[direction],
                        "size": {"X": 1, "B": 8, "W": 16, "D": 32, "L": 64}[
                            iec_size_prefix
                        ],
                        "IEC_type": iec_type,
                        "var_name": c_loc_name,
                        "location": iec_size_prefix
